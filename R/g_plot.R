...
Permute_positives_g = function(data, g_list, cell_type, sims = 100){
  ...
  data = data %>%
    mutate(
      Xloc = as.numeric(Xloc), 
      Yloc = as.numeric(Yloc)
    )
  
  # Create spatial point pattern (ppp) with defined window
  spatial_window <- owin(xrange = range(data$Xloc, na.rm = TRUE), 
                         yrange = range(data$Yloc, na.rm = TRUE))
  
  ppp_data <- ppp(x = data$Xloc, 
                  y = data$Yloc, 
                  window = spatial_window)
  
  # Use ppp_data instead of raw data in Gest() or other spatial functions
...
NN_G <- function(data, cell_type, alpha=0.05, sims = 100)
{
  ...
  w <- owin(xrange = range(loc$Xloc, na.rm = TRUE), 
            yrange = range(loc$Yloc, na.rm = TRUE))
  po_pp <- ppp(x= loc$Xloc, y= loc$Yloc, window = w)
